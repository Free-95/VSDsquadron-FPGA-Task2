...
**Path to `simple_gpio_output_ip.v`:** `./basicRISCV/RTL/`
...
**Path to `gpio.c`:** `./basicRISCV/Firmware/`
...
- Create `sim_cells.v` to mock Lattice iCE40 specific blocks (`SB_HFOSC`).
	...
	**Path to `sim_cells.v`:** `./basicRISCV/RTL/`
...
The access mechanism relies on **Memory Mapped I/O**. This means the CPU does not have special GPIO instructions. Instead, it treats hardware IP exactly like a location in RAM.

### The Address Map
First, the software and hardware must agree on a specific address to communicate.
* **The Address:** `0x400004`
* **Base (`0x400000`):** This upper bit tells the SoC, "This is an IO operation, not RAM."
* **Offset (`0x04`):** This lower bit tells the SoC, "Select the GPIO IP, not the UART."

Thus in `io.h`, `IO_OUT(port, val)` says: "Write data `val` to address `0x400004`."
...
- **Step 2 (Detect GPIO Access):** The CPU asserts the specific offset bit for IP. The code `localparam IO_GPIO_bit = 0;` targets the first bit of the *word* address (which corresponds to offset 4 in bytes).
- **Step 3 (Generate Select Signal):** The "Chip Select" signal is generated by combining these:
	```verilog
	wire gpio_sel = isIO & mem_wordaddr[IO_GPIO_bit];
	```
	If `gpio_sel` is HIGH, the IP knows it is being talked to.


### The Write Operation (CPU → LED)
When `gpio.c` executes `counter++` and writes to the LED, the following hardware sequence occurs:

1. **Data Setup:** The CPU places the counter value on the *Write Data Bus* (`mem_wdata`).
2. **Write Enable:** The CPU asserts the *Write Strobe* (`mem_wstrb`), signaling valid data is ready.
3. **IP Latch:** Inside `simple_gpio_output_ip.v` module, the following logic triggers:
	```verilog
	if (i_sel && i_we) begin
    	storage <= i_wdata;
	end
	```
	Because `gpio_sel` is High and `mem_wstrb` (connected to `i_we`) is High, the `storage` register updates.
4. **Physical Output:** The `storage` register is continuously wired to the physical pins (`assign o_gpio = storage[3:0]`), so the LEDs light up immediately.


### The Read Operation (LED → CPU)
When `gpio.c` executes the verification check (`if (read_val != counter)`), the data flows in reverse:

1. **Request:** The CPU puts `0x400004` on the address bus but keeps the Write Strobe (`mem_wstrb`) *LOW*.
2. **IP Output:** The IP is always outputting its current value on the `o_rdata` line (`assign o_rdata = storage`).
3. **Multiplexer:** The SoC must choose which data to send back to the CPU (RAM? UART? GPIO?).
	```verilog
	wire [31:0] IO_rdata = 
    	       mem_wordaddr[IO_UART_CNTL_bit] ? { 22'b0, !uart_ready, 9'b0} :
        	   mem_wordaddr[IO_GPIO_bit]      ? gpio_rdata :     
            	                                32'b0;
	```
	Since the address matches the GPIO bit, the multiplexer routes `gpio_rdata` onto the main `mem_rdata` bus.
4. **CPU Capture:** The CPU reads this value into a register variable (`read_val`) for comparison.
___
